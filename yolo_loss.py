...
class YoloLoss(nn.Module):
    ...
    def get_no_object_loss(self, target_tensor, pred_tensor, no_object_mask):
        ...
        no_object_prediction_mask = torch.cuda.ByteTensor(no_object_prediction.size()).fill_(0) 
        for b in range(self.B):
            no_object_prediction_mask[:, 4 + b*5] = 1 
        no_object_prediction = no_object_prediction[no_object_prediction_mask]       
        no_object_target = no_object_target[no_object_prediction_mask]   
        no_object_loss = F.mse_loss(no_object_prediction, no_object_target, reduction='sum')
    ...
    def find_best_iou_boxes(self, box_target, box_pred):
        ...
        """
        contains_object_response_mask = torch.cuda.ByteTensor(box_target.size()).fill_(0)
        box_target_iou = torch.zeros(box_target.size()).cuda()                    

        for i in range(0, box_target.size(0), self.B):
            pred = box_pred[i:i+self.B] 
            pred_xyxy = Variable(torch.cuda.FloatTensor(pred.size())) 
            ...
            target_xyxy = Variable(torch.cuda.FloatTensor(target.size())) 
    ...
    def forward(self, pred_tensor,target_tensor):
        ...
        contains_object_target = target_tensor[contains_object_mask].contiguous().view(-1, 30)         
